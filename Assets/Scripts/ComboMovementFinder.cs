...
public class ComboMovementFinder : MonoBehaviour
{
    ...
    public Vector3 previousReading;
    ...
    private void Start()
    {
        initialPosition = Vector3.zero;//Input.mousePosition;
    ...
    void Update()
    {
        ...
        {
            //initialPosition = new Vector3(arduinoInput.xValue / 5.0f, arduinoInput.yValue / 5.0f);
            previousReading = Vector3.zero; //Input.mousePosition;
            return;
        }

        // Start Tracking A Position When We Start Moving
        //nextPosition = Input.mousePosition;
        currentReading = new Vector3(arduinoInput.xValue / 5.0f, arduinoInput.yValue / 5.0f);

        if (((currentReading.x >= 0.9f || currentReading.x <= -0.9f) && Sign(previousReading.x) != Sign(currentReading.x)) ||
            ((currentReading.y >= 0.9f || currentReading.y <= -0.9f) && Sign(previousReading.y) != Sign(currentReading.y)))
        {
            EDirections currentDirection = FindDirectionFromAccelerometer();
    ...
    private EDirections FindDirectionFromAccelerometer()
    {
        if(currentReading.y >= 0.9f || currentReading.y <= -0.9f)
        {
            // We Have A YAxis Match
            return currentReading.y > 0 ? EDirections.North : EDirections.South;
        }

        return currentReading.x > 0 ? EDirections.East: EDirections.West;
    }
